...
ax.set_xlabel(r'Utilization (\%)',labelpad=20.0,size=25)
...
for i in range(1, 10):
    ...
    results_EDAGMF_OPA = np.load(EDAGMF_OPA_name)

    SCAIR_OPA_name = './outputs/SCAIR-OPA10_r' + str(rmod) + '_s' + str(smod) + '.npy'
    results_SCAIR_OPA = np.load(SCAIR_OPA_name)

    SCAIR_RM_name = './outputs/SCAIR-RM10_r' + str(rmod) + '_s' + str(smod) + '.npy'
    results_SCAIR_RM = np.load(SCAIR_RM_name)

    SCEDF_name = './outputs/SCEDF10_r' + str(rmod) + '_s' + str(smod) + '.npy'
    results_SCEDF = np.load(SCEDF_name)

    SCRM_name = './outputs/SCRM10_r' + str(rmod) + '_s' + str(smod) + '.npy'
    results_SCRM = np.load(SCRM_name)
    ...
    for j in range (len(results_sim)-1):
        ...
        y3.append(int(results_sim[j][2]))
        y4.append(int(results_sim[j][3]))
        y5.append(int(results_EDAGMF_OPA[1][j]*100))
        y6.append(int(results_SCAIR_OPA[1][j]*100))
        y7.append(int(results_SCAIR_RM[1][j]*100))
        y8.append(int(results_SCEDF[1][j] * 100))
        y9.append(int(results_SCRM[1][j] * 100))
    ...
    ax.plot(x5, y5, color='maroon', label='EDAGMF-OPA', linewidth=2.0, marker='p', markersize=12, markevery=1,
            fillstyle='none')
