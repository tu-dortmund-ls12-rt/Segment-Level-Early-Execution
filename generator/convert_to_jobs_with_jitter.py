...
def main(argv):
    ...
    for i in range(5, 101, 5):
        ...
        jitters = np.load(jitter_name, allow_pickle=True)

        job_name = '../experiments/inputs/jobs_jitter/jobs_jitter_n' + str(ntasks) + '_m' + str(msets) + '_p' + str(
            ...
            for ntsk in range(0, ntasks):
                for per in range(0, 1000, int(tasksets_org[st][ntsk][-1])):
                    ...
                    for jb in range(len(tasksets_org[st][ntsk])-2):
                        job.append(int(tasksets_org[st][ntsk][jb] * np.random.uniform(lbd, ubd) * ratio))
                    # utilization for total executions (except the suspensions)
                    job.append(tasksets_org[st][ntsk][-2])
                    # period
                    job.append(int(tasksets_org[st][ntsk][-1] * ratio))
                    # starting time
                    job.append(int(per * ratio + jitters[st][ntsk] * ratio))
                    # deadline
                    job.append(int(per * ratio + tasksets_org[st][ntsk][-1] * ratio))
                    # id of task
                    job.append(int(ntsk))
                    jobs.append(job)
            jobs_set.append(jobs)
        # np.save(job_name, jobs_set)
        np.save(job_name, np.array(jobs_set, dtype=object))
