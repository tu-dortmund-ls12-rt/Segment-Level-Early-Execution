...
# RM scheduling
def rm_self_suspension(tasksets_org, hyper_period, deadlines):
...
# A heuristic partition algorithm
# distribute all the tasks among processors averagelly
# sort all the tasks of this taskset (ignore the graph) according to the utilizations decreasingly
def heuristic_p_edf(tasksets, processors, task_ids, resources_utilization, max_period, cs_order, deadlines):
    tasksets = copy.deepcopy(tasksets)
    # get the partition
    partitioned = grouping_taskset_avg(resources_utilization, processors, task_ids)

    if (p_edf(tasksets, processors, partitioned, max_period, cs_order, deadlines) == 0):
        return 0
...
def glb_edf_sched(tasksets, num_sets, processors, max_period, cs_order):
    tasksets = copy.deepcopy(tasksets)
    n_sets = num_sets
    ...
    for i in range(0, n_sets):
        ...
        deadlines = deadline_update(tasksets[i], cs_order[i])
        accept = glb_edf(tasksets[i], processors, max_period, cs_order[i], deadlines)
        # print (accept)
        accepted = accepted + accept
    # print (accepted)
    return accepted

def glb_edf_np_sched(tasksets, num_sets, processors, max_period, cs_order):
    tasksets = copy.deepcopy(tasksets)
    n_sets = num_sets
    processors = processors
    accepted = 0
    ...
    for i in range(0, n_sets):
        ...
        deadlines = deadline_update(tasksets[i], cs_order[i])
        accept = glb_edf_np(tasksets[i], processors, max_period, cs_order[i], deadlines)
...
def heuristic_p_edf_sched(tasksets, num_sets, processors, task_ids, tsk_util, max_period, cs_order):
    tasksets = copy.deepcopy(tasksets)
    n_sets = num_sets
    processors = processors
    accepted = 0
    ...
    for i in range(0, n_sets):
        ...
        deadlines = deadline_update(tasksets[i], cs_order[i])
        accept = heuristic_p_edf(tasksets[i], processors, task_ids, tsk_util[i], max_period, cs_order[i], deadlines)

        accepted = accepted + accept

    return accepted

def heuristic_p_edf_np_sched(tasksets, num_sets, processors, task_ids, tsk_util, max_period, cs_order):
    tasksets = copy.deepcopy(tasksets)
    n_sets = num_sets
    processors = processors
    accepted = 0
    ...
    for i in range(0, n_sets):
        ...
        deadlines = deadline_update(tasksets[i], cs_order[i])
        accept = heuristic_p_edf_np(tasksets[i], processors, task_ids, tsk_util[i], max_period, cs_order[i], deadlines)
...
##################################################################################
# collect the makespans according different algorithms

def glb_edf_makespan(tasksets, num_sets, processors, cs_order):
    tasksets = copy.deepcopy(tasksets)
    n_sets = num_sets
    processors = processors
    makecpan = []
    ...
    for i in range(0, n_sets):
        ...
        deadlines = deadline_update(tasksets[i], cs_order[i])
        ms = makespan_glb_edf(tasksets[i], processors, cs_order[i], deadlines)

        makecpan.append(ms)
    return makecpan

def glb_edf_np_makespan(tasksets, num_sets, processors, cs_order):
    tasksets = copy.deepcopy(tasksets)
    n_sets = num_sets
    processors = processors
    makecpan = []
    ...
    for i in range(0, n_sets):
        ...
        deadlines = deadline_update(tasksets[i], cs_order[i])
        ms = makespan_glb_edf_np(tasksets[i], processors, cs_order[i], deadlines)

        makecpan.append(ms)
    return makecpan

def heuristic_p_edf_makespan(tasksets, num_sets, processors, task_ids, tsk_util, cs_order):
    tasksets = copy.deepcopy(tasksets)
    n_sets = num_sets
    processors = processors
    makecpan = []
    ...
    for i in range(0, n_sets):
        ...
        deadlines = deadline_update(tasksets[i], cs_order[i])
        ms = makespan_heuristic_p_edf(tasksets[i], processors, task_ids, tsk_util[i], cs_order[i], deadlines)

        makecpan.append(ms)

    return makecpan

def heuristic_p_edf_np_makespan(tasksets, num_sets, processors, task_ids, tsk_util, cs_order):
    tasksets = copy.deepcopy(tasksets)
    n_sets = num_sets
    processors = processors
    makecpan = []
    ...
    for i in range(0, n_sets):
        ...
        deadlines = deadline_update(tasksets[i], cs_order[i])
        ms = makespan_heuristic_p_edf_np(tasksets[i], processors, task_ids, tsk_util[i], cs_order[i], deadlines)
